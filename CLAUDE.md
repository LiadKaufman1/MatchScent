...
- **About and "Report a problem" (owner, 2026-09-25):** `/about` (`AboutViews.tsx`, texts in `i18n.ts` under `about`, first person, written in the owner's name "Liad" from the little that is known: the personal lines are a draft the owner should correct) and `/report` (noindex). The report form (`ReportForm.tsx` -> server action `sendReport` in `report-actions.ts`: validates, honeypot, 5 per 10 minutes per IP, saves through the service key) writes to the locked table `site_reports` (`db-migrations/2026-09-v7-site-reports.sql`, owner runs it, dry run first; until it runs the form answers "could not send" and shows the contact email if `NEXT_PUBLIC_CONTACT_EMAIL` is set). `ReportLink` (footer of every page, and inside the price panel next to "no prices found" and under the price list) adds `?page=<path>` so the report says where the problem is. `/admin/reports` lists them (mark handled / delete). Never say in these pages where prices come from.
