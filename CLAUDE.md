...
- **The main goal of the site (owner, 2026-09-25): find where a perfume is cheapest to buy in Israel**, besides being a Fragrantica-style community. The home page says so (hero text + 3 steps) and the perfume page opens with the big price button right under the name. The button "Compare prices in {country}" opens **our own panel** (`PriceCompare.tsx`), never Google: visitors must not be sent to Google and the site never says where the prices come from. Behind the scenes (`src/lib/prices.ts`): Google Shopping searches through SerpApi (`SERPAPI_KEY`, server only) for "<brand> <name>" in the visitor's country - in Israel twice, plain and with the Hebrew word for perfume added, because the two answers list different stores and Google's answers also change from run to run - then, for up to 2 listings that several stores sell (`multiple_sources`), the product page (`google_immersive_product`), which names EVERY store with its own address and price (this is how KSP, Super-Pharm and others are found: the search row shows only one of them, a different one each time). Cleaned by `src/lib/price-offers.ts`: drops samples, gift sets, lotions, other versions (Intense/Elixir/Absolu/Extrait, other colours or model names), the opposite gender, look-alikes, anything with a blocked word, and in Israel every store that is not an Israeli one (by name or by its own address ending in .il); matches Hebrew-only titles by sound ("קוקו שאנל מדמוזל" = Coco Chanel Mademoiselle); a listing that leaves out only the colour is kept and marked "check the model"; testers are grouped apart; the panel lists all sizes cheapest first (each row says its size and price per 100 ml) and a size chip narrows it; stores that are not found can be searched directly (KSP, Super-Pharm from `stores.ts`), served by `GET /api/prices?k=<perfume key>&c=<country>` (the key must be a perfume/entry of the catalogue, so it cannot be used as an open search proxy). Rows whose store address is known link straight to it (tracking marks such as srsltid/utm_* removed); the other rows open `/go`, which asks `GET /api/prices/go` for the store's own page of that listing (one more lookup, made only when someone clicks; cached 24 h) and redirects. Answers are cached `PRICE_CACHE_HOURS` (72) in the platform's fetch cache + server memory + the CDN, so a perfume costs up to 4 searches (Israel: 2 searches + 2 product pages) per 3 days plus 1 per clicked store that has no known address. The browser keeps an answer 5 minutes (`Cache-Control`), the CDN 12 h (`Vercel-CDN-Cache-Control`). **SerpApi's free plan is 250 searches a month; live use needs a paid plan** (ask the owner before raising volume; images and prices share the same quota). Prices load only when the visitor hovers/touches/opens the button (never on page load, so crawlers cannot burn the quota); `/api/` and `/go` are disallowed in robots.txt. If no prices can be loaded the panel offers the direct store searches from `stores.ts`. Offline test of the cleaning rules: `node --no-warnings scripts/test-price-offers.mjs` against saved results in `data-import/serpapi-samples/` (made with `scripts/price-probe.mjs`, which costs searches; it never prints the key).
