...
- **International:** the store links a visitor sees depend on their country (Israel: Israeli shops and stores that ship to Israel; USA: US stores; UK: UK stores; same idea for the rest of the world). Start with Israel, USA, UK. The site is English only.
- **Real fragrance photos** are wanted (today all 130 perfumes reuse 6 Unsplash stock photos, and the old Google-hosted dupe thumbnails do not load). Photos must come from a source we are allowed to use (own photos, brand/retailer permission, or an affiliate product feed). Do NOT hotlink or copy images from Google or Fragrantica; ask the owner first.
- **SerpApi is on the free plan** (small monthly quota): do not add new SerpApi calls casually.
- **Live site:** https://match-scent-6ev8.vercel.app (Vercel project `match-scent-6ev8`). The owner is deleting the three other Vercel projects that were connected to this repo (`match-scent`, `match-scent1`, `match-scent-wvtu`); they all failed to build. All five environment variables must be set for both Production and Preview in Vercel, or previews fail.
- **Database cleanup:** `db-cleanup/2026-09-cleanup.sql` is a reviewed-by-owner-first script (dry run by default) that removes duplicate perfumes and the Google-sourced entries. The assistant must NOT run it; the owner runs it in the Supabase SQL editor.
