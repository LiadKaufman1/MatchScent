...
export async function generateMetadata({ searchParams }: Props) {
  ...
  return allPerfumesMetadata('en', !!(one(p.q) || one(p.gender) || one(p.kind) || one(p.page)));
}

export default async function Page({ searchParams }: Props) {
  const p = await searchParams;
  return <AllPerfumesView lang="en" q={one(p.q)} gender={asGender(one(p.gender))} kind={asKind(one(p.kind))} page={Number(one(p.page)) || 1} />;
}
